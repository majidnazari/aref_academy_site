...
import { GET_CONSULTANTS, CONSULTANT_REPORT } from './gql/query'
...
const ConsultantReport_2 = () => {
  ...
  const [search, setSearch] = useState<SearchConsultantReport>({
    consultant_id: undefined,
    static_date_from: undefined,
    static_date_to: undefined,
    //education_level: 0,
  })
  ...
  const { refetch } = useQuery(CONSULTANT_REPORT, {
    ...
    onCompleted(data) {
      setTotalReport(data.getConsultantReport)
    },
  })
  ...
  const handleSearch = (): void => {
    ...
    let refetchData: SearchConsultantReport = { ...search }
    //refetchData.education_level = refetchData.education_level === 0 ? undefined : refetchData.education_level
    refetchData.static_date_from = refetchData.static_date_from
      ? moment(refetchData.static_date_from).format('YYYY-MM-DD')
      : undefined
    refetchData.static_date_to = refetchData.static_date_to ? moment(refetchData.static_date_to).format('YYYY-MM-DD') : undefined
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={3} xl={3}>
              <LocalizationProvider dateAdapter={AdapterJalali}>
                <DatePicker
                  label="از تاریخ"
                  value={search.static_date_from || null}
                  onChange={(newValue) => {
                    if (newValue) {
                      setSearch({
                        ...search,
                        static_date_from: newValue as string,
                      })
                    }
                  }}
            ...
            <Grid item xs={12} sm={6} md={3} xl={3}>
              <LocalizationProvider dateAdapter={AdapterJalali}>
                <DatePicker
                  label="تا تاریخ"
                  value={search.static_date_to || null}
                  onChange={(newValue) => {
                    if (newValue) {
                      setSearch({
                        ...search,
                        static_date_to: newValue as string,
                      })
                    }
                  }}
                  renderInput={(params) => <TextField {...params} style={{ width: '100%' }} />}
...
export default ConsultantReport_2
