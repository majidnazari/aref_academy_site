...
const EditAcceptorConsultantFinancialStatus = ({
...
}: //closeStudentStatusDialog,
{
  ...
  const handleAdd = () => {
    if ((studentStatus === 'refuse_pending' || studentStatus === 'fire_pending') && financialDescription.trim() == '') {
    ...
    <Dialog open={open} onClose={handleCancel}>
      ...
      <DialogContent>
        ...
        <FormControl sx={{ width: '50%', alignItems: 'left', margin: '2px' }}>
          <InputLabel id="session-time-label"> پذیرشگر مشاور</InputLabel>
          <Select
            labelId="session-time-label"
            value={managerStatus}
            onChange={(e) => {
            ...
            input={<OutlinedInput label=" پذیرشگر مشاور" />}
            fullWidth
          >
...
export default EditAcceptorConsultantFinancialStatus
