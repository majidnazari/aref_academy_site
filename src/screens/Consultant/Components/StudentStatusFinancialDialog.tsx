...
import { GET_A_CONSULTANT_TIME_TABLE, GET_STUDENTS, GET_CONSULTANT_FINANCIALS } from '../gql/query'
import {
  Autocomplete,
  CircularProgress,
  FormControl,
...
const StudentStatusFinancialDialog = ({
  ...
  const { refetch: refetchStudents, loading: studentLoading } = useQuery(GET_STUDENTS, {
    variables: {
      first: 1,
      page: 1,
      full_name: '',
    ...
    onCompleted: (data) => {
      const tmp = data.getStudents.data[0].first_name + ' ' + data.getStudents.data[0].last_name
  ...
  const handleAdd = () => {
    ...
    //   return null
    // }
    if (['refused', 'fired', 'refuse_pending', 'fire_pending'].indexOf(studentStatus) > -1 && financialDescription.trim() == '') {
      showError('  در صورت انتخاب دانش آموز روی وضعیت انصراف و یا اخراج توضیحات باید وارد شود. ')
      return null
    }
        ...
        <>
          ...
          {studentStatus === 'refused' || studentStatus === 'fired' ? (
            <DialogContent>
              <Grid item xs={12} sm={6} lg={6} md={6} xl={6}>
                <FormControl sx={{ width: '100%' }}>
                  <InputLabel id="financial-refused-status-label"> وضعیت اخراج دانش آموز </InputLabel>
                  <Select
