...
import { Button, Container, Stack, Switch, TableContainer } from '@mui/material'
...
const ConsultantTimeTable = () => {
  ...
  const [timeTable, setTimeTable] = useState<getConsultantDefinitionDetailsData[]>([])
  const [open, setOpen] = useState<boolean>(false)
  const [dialogconsultantTimeTableId, setDialogConsultantTimeTableId] = useState<string>()
  ...
  const [insertMultiConsultantTimes] = useMutation(CREATE_CONSULTANT_DEFINITION_DETAIL)
  const [insertCopyMultiConsultantTimes] = useMutation(CPOY_TIME_TABLE_OF_CONSULTANT)

  const [deleteConsultantTimeTableStudentId] = useMutation(DELETE_CONSULTANTN_DEFINITION_STUDENT_ID)

  const [deleteOneSessionTimeTable] = useMutation(DELETE_ONE_SESSION_OF_TIME_TABLE)

  const [copyStudentToNextWeek] = useMutation(COPY_STUDENT_TO_NEXT_WEEK)
  const [copyOneDayConsultantTimeTable] = useMutation(COPY_ONE_DAY_CONSULTANT_TIME_TABLE)

  const [ConsultantTimeTableStudentStatus] = useMutation(UPDATE_CONSULTANT_DEFINITION_DETAIL_STUDENT_ID)
  ...
  const { data: consultantDetails } = useQuery(GET_A_USER, {
    ...
    onCompleted: (data) => {
      ...
      setConsultantFullName(data.getUser.first_name + ' ' + data.getUser.last_name)
    },
  })
  ...
  const { refetch: refetchConsultantStudentsByDefinitionId } = useQuery(GetConsultantStudentsByDefinitionId, {
    variables: {
      id: -1,
    },
    onCompleted: (data) => {
  ...
  const changeStudentStatus = (id: string, student_id: string, student_status: string) => {
    ...
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            {week === 'Current' || true ? (
              <Stack direction="row" spacing={1}>
                ...
                <FormControl sx={{ width: '100%' }}>
                  ...
                  >
                    <MenuItem value="Current">جاری {momentj(current_date).subtract(2, 'days').format('jYYYY/jMM/jDD')}</MenuItem>
                    <MenuItem value="Next">یک هفته بعد {momentj(next_date).subtract(2, 'days').format('jYYYY/jMM/jDD')}</MenuItem>
                    <MenuItem value="Next2week">
                      دو هفته بعد {momentj(next2week_date).subtract(2, 'days').format('jYYYY/jMM/jDD')}
                    </MenuItem>
                    <MenuItem value="Next3week">
                      سه هفته بعد {momentj(next3week_date).subtract(2, 'days').format('jYYYY/jMM/jDD')}
                    </MenuItem>
                    <MenuItem value="Next4week">
                      چهار هفته بعد {momentj(next4week_date).subtract(2, 'days').format('jYYYY/jMM/jDD')}
                    </MenuItem>
                  </Select>
                  {error.days ? <FormHelperText error>{error.days}</FormHelperText> : ''}
                </FormControl>
      ...
      >
        ...
        <Table aria-label="customized table">
          ...
          <TableBody>
            {timeTable.map((element: getConsultantDefinitionDetailsData, index: number) => (
              <TableRow key={index}>
                <StyledTableCell align="center">
                  <>
                    <Box className="dateOfWeekString">
                      {moment_jalali(element.date?.toString()).format('jYYYY/jMM/jDD')} <br />
                      {convertDayIntoShamsi(moment_jalali(element.date).format('dddd'))}
                      <span></span>
                      <span></span>
                      <span></span>
                      <span></span>
                ...
                <StyledTableCell align="left">
                  <Box sx={{ display: 'flex', flexDirection: 'row' }}>
                    {element.details?.map((detail: detailsData, index_details: number) => (
                      <Box key={index_details}>
                        ...
                        <Box
                          sx={
                            convertStudentStatusTheme(detail?.student_status)
                            // {
                            //   display: "flex",
                            //   flexDirection: "column",
                            //   justifyContent: "space-between",
                        ...
                        >
                          <Box display="flex" justifyContent="space-between" alignItems="center">
                            ...
                            {detail?.student_id ? (
                              <Button
                                startIcon={<ManageAccountsIcon sx={{ textAlign: 'left' }} />}
                                sx={{
                                  color: 'black',
                                  fontSize: 13,
                                  fontWeight: 800,
                            ...
                            <Box>
                              ...
                              <Box display={'flex'} justifyContent={'space-between'} mt={1}>
                                <Button
                                  ...
                                  // }}
                                  onClick={() => {
                                    changeStudentStatus(detail.id, detail.student_id, 'present')
                                  }}
                                >
                                  {'حاضر '}
                                </Button>
                                <Button
                                  ...
                                  onClick={() => {
                                    changeStudentStatus(detail.id, detail.student_id, 'absent')
                                  }}
                                >
                                  {'غایب '}
                                </Button>
                              ...
                              <Box>
                                ...
                                >
                                  <Select
                                    ...
                                    value={detail.student_status}
                                    onChange={(e) => {
                                      changeStudentStatus(detail.id, detail.student_id, e.target.value)
                                      // alert(detail.id);
                                  ...
                                  >
                                    ...
                                    <MenuItem value="dellay5">تاخیر ۵ دقیقه </MenuItem>
                                    <MenuItem value="dellay10">تاخیر ۱۰ دقیقه</MenuItem>
                                    <MenuItem value="dellay15">تاخیر ۱۵ دقیقه</MenuItem>
                                    <MenuItem value="dellay15more">تاخیر بیشتر از ۱۵ دقیقه</MenuItem>
                                  </Select>
                                </FormControl>
                              </Box>
                              {detail?.student_id ? (
                                    ...
                                    onClick={() => {
                                      showConfirm(async () =>
                                        copyStudentToNextWeek({
                                          ...
                                          showSuccess('کپی با موفقیت انجام شد.')
                                          refetch()
                                          changeWeek(week)
                                          //showPreviousWeekBeforeRefresh();
                                        })
                              ...
                              <Box>
                                <Switch
                                  checked={detail.single_meet === true ? detail.single_meet : false}
                                  size="small"
                                  onClick={() => {
                                    ...
                                    ConsultantTimeTableStudentStatus({
                                      ...
                                      showSuccess('وضعیت  تک جلسه تغییر کرد.')
                                      refetch()
                                      changeWeek(week)
                                      //showPreviousWeekBeforeRefresh();
                                    })
                              ...
                              <Box>
                                <Switch
                                  checked={detail.remote === true ? detail.remote : false}
                                  size="small"
                                  onClick={() => {
                                    //alert(detail.remote);
                                    ConsultantTimeTableStudentStatus({
                              ...
                              <Box
                                bgcolor={converConsultantFinancialCode(detail?.consultant_financial?.financial_status)}
                                sx={{
                                  my: 1,
                                  px: 1,
                                }}
                                borderRadius={1}
                              >
                                {converConsultantFinancial(detail?.consultant_financial?.financial_status)}
                              </Box>
                                ...
                                <Box>
                                  ...
                                  >
                                    {detail?.student_status_updated_at
                                      ? momentj(detail.student_status_updated_at).format('jYYYY/jMM/jDD-HH:mm')
                                      : null}
                                  </Box>
                                  {/* {detail?.student_status_updated_at} */}
                                </Box>
