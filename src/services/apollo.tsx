...
const logoutLink = onError((networkError) => {
  if (networkError.response?.errors) {
    if (
      networkError.response.errors[0].message === "Unauthenticated." ||
    ...
    if (
      networkError.response.errors[0].message ===
      "You need to pass authorization header"
    ) {
      return;
    }
    ...
    if (networkError.response.errors[0].extensions.validation) {
      message = "";
      const tmpValidation: any =
        networkError.response.errors[0].extensions.validation;
